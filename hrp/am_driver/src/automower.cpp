...
Automower::Automower(const ros::NodeHandle& nodeh)
{
    // Init attributes
    nh = nodeh;
    lin_vel = 0.0;
    ...
    cmd_sub = nh.subscribe("cmd_mode", 5, &Automower::modeCallback, this);
    ...
    imuResetSub = nh.subscribe("imu_reset", 1, &Automower::imuResetCallback, this);

    velocity_sub = nh.subscribe("cmd_vel", 1, &Automower::velocityCallback, this);
...
bool Automower::setup()
{
    ...
    if (serialFd < 0)
    {
        ROS_ERROR("Automower::Serial port open:");
        return false;
    }
    ...
    /*
            ...
            {
                    ROS_ERROR("Automower::setup():tcgetattr():");
                    close(serialFd);
                    serialFd = -1;
                    return false;
            }
            ...
            {
                    ROS_ERROR("Automower::setup():tcflush():");
                    close(serialFd);
                    serialFd = -1;
                    return false;
            }
            ...
            {
                    ROS_ERROR("Automower::setup():tcsetattr():");
                    close(serialFd);
                    serialFd = -1;
                    return false;
            }
...
void Automower::imuResetCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
...
void Automower::velocityCallback(const geometry_msgs::Twist::ConstPtr& vel)
{
...
void Automower::modeCallback(const std_msgs::UInt16::ConstPtr& msg)
{
    if (msg->data < 0x90)
    {
        // Not for us...
    ...
    // ROS_INFO("Automower::cmd_mode: %d", msg->data);
}
...
bool Automower::initAutomowerBoard()
{
    ...
    if (reclength == 0)
    {
        ROS_ERROR("Automower::Failed to init mower!!!");
        return false;
    }
    ...
    ROS_INFO("Automower::WHEEL_SPEED_TIMER_TICK = %d", WHEEL_SPEED_TIMER_TICK);
...
int Automower::sendMessage(unsigned char* msg, int len, unsigned char* ansmsg, int maxAnsLength, bool retry)
{
    ...
    */
    ...
    while (ansmsg[cnt] != 0x02)
    {
        ...
        if (res <= 0)
        {
            // ROS_WARN("Automower::Failed to get STX...sleeping?");
            serialPortState = AM_SP_STATE_OFFLINE;
            return 0;
        }
    }
    ...
    */
    ...
    {
        // FAILED
        ROS_WARN("Automower::Failed STX");
        return 0;
    }
    if (ansmsg[cnt - 1] != 0x03)
    {
        // FAILED
        ROS_WARN("Automower::Failed ETX");
        return 0;
    }
...
int Automower::twosComp(int val, int bits)
{
...
double Automower::regulateVelocity(ros::Duration dt, double wanted_vel, double actual_vel)
{
...
void Automower::doAccelerationControl(ros::Duration dt)
{
...
bool Automower::sendNavigation(signed short lv, signed short rv)
{
    ...
    // ROS_INFO("Automower::lv = %d, rv = %d", lv, rv);
    // ROS_INFO("Automower::leftPulses = %d, rightPulses = %d", leftPulses, rightPulses);

    return true;
}

bool Automower::update(ros::Duration dt)
{
    ...
    if (serialPortState == AM_SP_STATE_CONNECTED)
    {
        ...
        if (!res)
        {
            ROS_ERROR("Automower::Failed Navigation message => OFFLINE!");
            serialPortState = AM_SP_STATE_OFFLINE;
            return false;
        }
    }
