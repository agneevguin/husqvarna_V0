 ...
 * 
 * NOTE: Heavily borrowed&rewritten from:
 * https://github.com/spauka/SLAM/blob/master/occupancygrid/quadtree.h
 * 
 */
...
public:
	...
	{
		float D = Cell::intersectSquare((float)x,(float)y,1.0f,X,Y,rx,ry);
	...
	static float intersectSquare(float x, float y, float size, float X, float Y, float rx, float ry)
	{
		if (rx > 0)
		{
			float dx = x - X;
...
public:
	...
	{
		return Cell::intersectSquare((float)x,(float)y,(float)size,X,Y,rx,ry);
	}
...
public:
  ...
  std::set<Cell> unoccupied;
	...
	nav_msgs::OccupancyGrid* NewOccupancyGrid()
	{
