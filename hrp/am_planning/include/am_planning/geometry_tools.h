...
*/
template <typename T, typename S> double pointsDistance(T one, S two)
{
...
*/
template <typename T, typename S> bool pointsNearby(T one, S two, double proximity)
{
...
*/
template <typename T> bool pointInPolygon(T point, geometry_msgs::Polygon polygon)
{
    int cross = 0;
    for (int i = 0, j = polygon.points.size() - 1; i < polygon.points.size(); j = i++)
    {
...
*/
template <typename T, typename S> double yawOfVector(T origin, S end)
{
