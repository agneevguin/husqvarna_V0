#include <am_confinement/hrp_geometry_helpers.h>
...
void shrinkPolygonCgal(double shrinkFactor, geometry_msgs::Polygon& input, geometry_msgs::Polygon& output)
{
    ...
    FT lOffset = shrinkFactor;
    if (lOffset > 0.0001)
    {
        Polygon_2 cgalInput;
        cgalPolyFromRos(input, cgalInput);
        // Convert from ROS-polygon to CGAL polygon
        //printCgalPoly(cgalInput);
        PolygonPtrVector inner_offset_polygons = CGAL::create_interior_skeleton_and_offset_polygons_2(lOffset, cgalInput);
        PolygonPtrVector::iterator it;
        it = inner_offset_polygons.begin();
        if (it != inner_offset_polygons.end())
        {
            PolygonPtr pp  = *it;
            rosPolyFromCgal(*pp,output);
        }
        else
        {
            std::cout << "Failed to shrink Polygon" << std::endl;
        }
    }
    else
    {
